...
[![Tested on PF2e](https://img.shields.io/badge/Tested%20on-PF2e-blue.svg)](https://github.com/foundryvtt/pf2e)
...
While **system-agnostic** (supporting core Foundry rolls on any system), `pp-dice` is **tested on Pathfinder 2e** and features deep, dedicated integration (Party sheet detection, Fortune & Misfortune traits, and degree of success automation).
...
> 🎲 **System Compatibility**: System-agnostic (core Foundry rolls on any system) — **tested on Pathfinder 2e**.
...
- **Keyboard-First Dialog**:
  ...
  - Press `Enter` with values to confirm physical roll.
  - Press `Enter` on an empty box for an instant digital roll.
  - Press `Escape` or `R` (or click "Roll Digital") for instantaneous fallback to digital RNG (even while inputs are focused).
...
- **Client-Side Evaluation Model**: Interception runs locally on the rolling client (typically the GM screen in an in-person common display setup). Whoever initiates or evaluates the roll provides the physical dice results.
